...
2. [代码格式](#代码格式)
3. [注释规范](#注释规范)
4. [接口层面规范](#接口层面规范)（含 Dubbo 接口参数变更规范）
5. [分页规范](#分页规范)
6. [异常处理规范](#异常处理规范)
7. [类和方法设计](#类和方法设计)
8. [集合使用](#集合使用)
...
**核心原则：Dubbo 对外接口的参数类型一旦发布，修改必须谨慎，变更前需确认所有下游调用方。**

#### 高风险变更（需用户确认后才能执行）

以下类型变更会导致下游服务编译报错，**必须暂停操作，向用户明确说明影响，经确认后再修改**：

| 变更类型 | 示例 | 影响 |
|---------|------|------|
| 泛型类型替换 | `List<Long>` → `List<String>` | 下游编译报错 |
| 基础类型替换 | `Long` → `String`, `Integer` → `Long` | 下游编译报错 |
| 参数类型收窄 | `Object` → `Long` | 下游编译报错 |
| 删除参数 | 移除已有参数 | 下游编译报错 |
| 修改返回值类型 | `UserDTO` → `UserVO` | 下游编译报错 |
...
1. 识别到高风险变更时，停止修改
2. 告知用户：
   - 变更内容（哪个接口、哪个参数、从什么类型改为什么类型）
   - 影响范围（Dubbo 下游调用方需同步升级）
   - 建议方案（见下文兼容性方案）
3. 用户明确确认后，再执行修改
```
...
以下场景属于服务内部调用，不影响外部调用方，可直接修改：

- `@RestController` HTTP 接口（调用方通过 JSON 序列化，类型容错性强）
- Service / Manager / Mapper 等内部实现类方法
- 仅项目内部调用的 Service 接口
- 未发布到注册中心的接口

#### 兼容性变更建议（Dubbo 接口优先使用）

```java
// ❌ 破坏性变更 - 直接修改类型
List<Long> getUserIds();  →  List<String> getUserIds();  // 下游编译报错

// ✅ 兼容性方案1 - 新增方法，保留旧方法（加 @Deprecated 标记）
/**
 * @deprecated 请使用 getUserStringIds() 代替
 */
@Deprecated
List<Long> getUserIds();

List<String> getUserStringIds();

// ✅ 兼容性方案2 - 使用新版本接口（版本号升级）
...
满足以下任一条件，即为需要谨慎变更的对外接口：

- 接口类位于 `facade` 模块或 `api` 模块
- 接口类上有 `@DubboService` 注解
- 接口类被 `@DubboReference` 引用（在其他服务中）
- 接口包路径含 `facade`、`api`、`provider`
